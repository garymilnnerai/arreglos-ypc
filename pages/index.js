      ...
      <div style={{ maxWidth: 420, margin: '0 auto', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
                ...
                return (
                  <div key={b.n} onClick={() => { setModalBQ({ num: b.n }); setForm({}); setSelMonthOpt(''); setSelSunday(''); }}
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      {ALL_B_GI[b.n] && <div style={{ fontSize: 13, fontWeight: 300, color: D.text3, opacity: 0.75, marginTop: 2 }}>{ALL_B_GI[b.n]}</div>}
                      ...
                      {(a?.bqNum || a?.customTitle) && <><div style={{ fontSize: 14, color: D.text2 }}>{a.name || '—'} · {a.cong || '—'}{a.tel ? ' · ' + a.tel : ''}</div>
                        {a.customTitle ? (
                          ...
                          <>
                            ...
                            {ALL_B_GI[a.bqNum] && <div style={{ fontSize: 12, color: D.text3, opacity: 0.75, marginTop: 1 }}>{ALL_B_GI[a.bqNum]}</div>}
                          </>
                        )}</>}
                                ...
                                <div>
                                  ...
                                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 8 }}>
                                    ...
                                    <select value={e.bqNum} onChange={ev => updateEntry(idx, 'bqNum', ev.target.value)} disabled={!e.speaker}
                                      ...
                                      {bqs.map(n => <option key={n} value={n}>{n} — {ALL_B[n] || ''}{ALL_B_GI[n] ? ` / ${ALL_B_GI[n]}` : ''}</option>)}
                                    </select>
                                  </div>
        ...
        {modalBQ && (
          <Overlay onClose={() => setModalBQ(null)} D={D}>
            ...
            <div style={{ fontSize: 11, color: D.text2, marginBottom: 18, lineHeight: 1.4 }}>{ALL_B[modalBQ.num]}
              {ALL_B_GI[modalBQ.num] && <div style={{ marginTop: 4, opacity: 0.75 }}>{ALL_B_GI[modalBQ.num]}</div>}
            </div>
              ...
              <>
                <div style={{ marginBottom: 13 }}>
                  ...
                  <div onClick={() => { setModalBQSel(true); setBqSelSearch(''); setModalBQCustom(false); }}
                    ...
                    {sunBQNum ? (<>{sunBQNum} — {ALL_B[sunBQNum]}{ALL_B_GI[sunBQNum] && <div style={{ fontSize: 12, opacity: 0.75, marginTop: 2 }}>{ALL_B_GI[sunBQNum]}</div>}</>) : sunCustomTitle ? `📌 ${sunCustomTitle}` : 'Seleccionar bosquejo →'}
                  </div>
                </div>
                <FField label="Conferenciante" placeholder="Nombre y apellido" value={form.name || ''} onChange={v => setForm(f => ({ ...f, name: v }))} D={D} />
                <FField label="Congregación" placeholder="De dónde proviene" value={form.cong || ''} onChange={v => setForm(f => ({ ...f, cong: v }))} D={D} />
                      ...
                      return (
                        <div key={b.n} onClick={() => { setSunBQNum(b.n); setSunCustomTitle(''); setModalBQSel(false); }}
                          ...
                          <div>
                            ...
                            {ALL_B_GI[b.n] && <div style={{ fontSize: 13, color: D.text3, opacity: 0.75, marginTop: 2, lineHeight: 1.3 }}>{ALL_B_GI[b.n]}</div>}
