...
export async function readAssignments() {
  try {
    ...
    const [res1, res2, res3, res4] = await Promise.all([
      sheets.spreadsheets.values.get({ spreadsheetId: SHEET_ID, range: `${SHEET_NAME}!A2:G` }),
      sheets.spreadsheets.values.get({ spreadsheetId: SHEET_ID, range: `Contactos!A2:D` }).catch(() => ({ data: { values: [] } })),
      sheets.spreadsheets.values.get({ spreadsheetId: SHEET_ID, range: `Salidas!A2:G` }).catch(() => ({ data: { values: [] } })),
      sheets.spreadsheets.values.get({ spreadsheetId: SHEET_ID, range: `Roles!A2:C` }).catch(() => ({ data: { values: [] } })),
    ]);
    const rows = res1.data.values || [];
    const assignments = {};
    rows.forEach(([date, bqNum, name, cong, tel, asamblea, customTitle]) => {
      if (!date) return;
      if (asamblea === 'true') {
    ...
    const outgoing = {};
    outgoingRows.forEach(([mcKey, speaker, bqNum, cong, day, time, date]) => {
      if (!mcKey) return;
      if (!outgoing[mcKey]) outgoing[mcKey] = [];
      outgoing[mcKey].push({ speaker: speaker||'', bqNum: bqNum||'', cong: cong||'', day: day||'dom', time: time||'', date: date||'' });
    });
    const rolesRows = res4.data.values || [];
    const roles = {};
    rolesRows.forEach(([date, presidente, lector]) => {
      if (!date) return;
      roles[date] = { presidente: presidente||'', lector: lector||'' };
    });
    return { assignments, monthContacts, outgoing, roles };
  } catch (e) {
...
export async function writeAssignments(assignments, monthContacts = {}, outgoing = {}, roles = {}) {
  try {
    const auth = getAuth();
    const sheets = google.sheets({ version: 'v4', auth });
    ...
    const rows = Object.entries(assignments).map(([date, a]) => {
      ...
      return [date, a.bqNum || '', a.name || '', a.cong || '', a.tel || '', 'false', a.customTitle || ''];
    });
    if (rows.length > 0) {
    ...
    try {
      await sheets.spreadsheets.values.clear({ spreadsheetId: SHEET_ID, range: `Salidas!A2:G` });
    } catch {
      try {
        await sheets.spreadsheets.batchUpdate({
          spreadsheetId: SHEET_ID,
        ...
        await sheets.spreadsheets.values.update({
          spreadsheetId: SHEET_ID, range: 'Salidas!A1:G1',
          valueInputOption: 'RAW', requestBody: { values: [['Mes','Conferenciante','Bosquejo','Congregación','Día','Horario','Fecha']] },
        });
      } catch {}
    }
    ...
    Object.entries(outgoingParam).forEach(([mcKey, entries]) => {
      entries.forEach(e => {
        if (e.speaker || e.cong) outgoingRows.push([mcKey, e.speaker||'', e.bqNum||'', e.cong||'', e.day||'dom', e.time||'', e.date||'']);
      });
    });
    ...
    // Write roles
    try {
      await sheets.spreadsheets.values.clear({ spreadsheetId: SHEET_ID, range: `Roles!A2:C` });
    } catch {
      try {
        await sheets.spreadsheets.batchUpdate({
          spreadsheetId: SHEET_ID,
          requestBody: { requests: [{ addSheet: { properties: { title: 'Roles' } } }] },
        });
        await sheets.spreadsheets.values.update({
          spreadsheetId: SHEET_ID, range: 'Roles!A1:C1',
          valueInputOption: 'RAW', requestBody: { values: [['Fecha','Presidente','Lector']] },
        });
      } catch {}
    }
    const rolesParam = roles || {};
    const rolesRows = Object.entries(rolesParam)
      .filter(([, r]) => r.presidente || r.lector)
      .map(([date, r]) => [date, r.presidente||'', r.lector||'']);
    if (rolesRows.length > 0) {
      await sheets.spreadsheets.values.update({
        spreadsheetId: SHEET_ID, range: 'Roles!A2',
        valueInputOption: 'RAW', requestBody: { values: rolesRows },
      });
    }
...
export async function initSheet() {
  try {
    ...
    await sheets.spreadsheets.values.update({
      ...
      requestBody: {
        values: [['Fecha', 'Bosquejo Nro', 'Conferenciante', 'Congregación', 'Teléfono', 'Asamblea', 'Título especial']],
      },
    });
    return true;
  } catch (e) {
